...
class GUI:
    ...
    def setup_ip_expansion_tab(self):
        ...
        ttk.Radiobutton(frame_source, text="Локальные файлы", variable=self.var_source, value="files").pack(side='left', padx=5)
        ttk.Radiobutton(frame_source, text="URL", variable=self.var_source, value="urls").pack(side='left', padx=5)
        ...
        # Список для файлов
        frame_files = ttk.Frame(frame_input)
        frame_files.pack(fill='both', expand=True, padx=5, pady=5)
        scrollbar_files = ttk.Scrollbar(frame_files)
        scrollbar_files.pack(side='right', fill='y')
        self.listbox_ip_files = tk.Listbox(frame_files, yscrollcommand=scrollbar_files.set)
        self.listbox_ip_files.pack(side='left', fill='both', expand=True)
        scrollbar_files.config(command=self.listbox_ip_files.yview)
        
        frame_file_buttons = ttk.Frame(frame_files)
        frame_file_buttons.pack(fill='x', pady=5)
        btn_add_files = ttk.Button(frame_file_buttons, text="Добавить файлы", command=self.add_ip_files)
        btn_add_files.pack(side='left', padx=5)
        btn_clear_files = ttk.Button(frame_file_buttons, text="Очистить список", command=self.clear_ip_files)
        btn_clear_files.pack(side='left', padx=5)
        
        # Список для URL
        frame_urls = ttk.Frame(frame_input)
        frame_urls.pack(fill='both', expand=True, padx=5, pady=5)
        scrollbar_urls = ttk.Scrollbar(frame_urls)
        scrollbar_urls.pack(side='right', fill='y')
        self.listbox_ip_urls = tk.Listbox(frame_urls, yscrollcommand=scrollbar_urls.set)
        self.listbox_ip_urls.pack(side='left', fill='both', expand=True)
        scrollbar_urls.config(command=self.listbox_ip_urls.yview)
        
        frame_url_buttons = ttk.Frame(frame_urls)
        frame_url_buttons.pack(fill='x', pady=5)
        self.entry_ip_url = ttk.Entry(frame_url_buttons)
        self.entry_ip_url.pack(side='left', fill='x', expand=True, padx=5)
        btn_add_url = ttk.Button(frame_url_buttons, text="Добавить URL", command=self.add_ip_url)
        btn_add_url.pack(side='left', padx=5)
        btn_clear_urls = ttk.Button(frame_url_buttons, text="Очистить список", command=self.clear_ip_urls)
        btn_clear_urls.pack(side='left', padx=5)
    ...
    def setup_cidr_optimization_tab(self):
        ...
        ttk.Radiobutton(frame_source, text="Локальные файлы", variable=self.var_opt_source, value="files").pack(side='left', padx=5)
        ttk.Radiobutton(frame_source, text="URL", variable=self.var_opt_source, value="urls").pack(side='left', padx=5)
        ...
        scrollbar_files = ttk.Scrollbar(frame_files)
        scrollbar_files.pack(side='right', fill='y')
        self.listbox_opt_files = tk.Listbox(frame_files, yscrollcommand=scrollbar_files.set)
        self.listbox_opt_files.pack(side='left', fill='both', expand=True)
        scrollbar_files.config(command=self.listbox_opt_files.yview)
        
        frame_file_buttons = ttk.Frame(frame_files)
        frame_file_buttons.pack(fill='x', pady=5)
        btn_add_files = ttk.Button(frame_file_buttons, text="Добавить файлы", command=self.add_opt_files)
        btn_add_files.pack(side='left', padx=5)
        btn_clear_files = ttk.Button(frame_file_buttons, text="Очистить список", command=self.clear_opt_files)
        btn_clear_files.pack(side='left', padx=5)
        ...
        scrollbar_urls = ttk.Scrollbar(frame_urls)
        scrollbar_urls.pack(side='right', fill='y')
        self.listbox_opt_urls = tk.Listbox(frame_urls, yscrollcommand=scrollbar_urls.set)
        self.listbox_opt_urls.pack(side='left', fill='both', expand=True)
        scrollbar_urls.config(command=self.listbox_opt_urls.yview)
        
        frame_url_buttons = ttk.Frame(frame_urls)
        frame_url_buttons.pack(fill='x', pady=5)
        self.entry_opt_url = ttk.Entry(frame_url_buttons)
        self.entry_opt_url.pack(side='left', fill='x', expand=True, padx=5)
        btn_add_url = ttk.Button(frame_url_buttons, text="Добавить URL", command=self.add_opt_url)
        btn_add_url.pack(side='left', padx=5)
        btn_clear_urls = ttk.Button(frame_url_buttons, text="Очистить список", command=self.clear_opt_urls)
        btn_clear_urls.pack(side='left', padx=5)
    ...
    def expand_ips(self):
        ...
        if source == "text":
            ...
            all_ips.extend(ips)
            if saved:
                self.text_ip_output.insert(tk.END, f"IP сохранены в: {output_path}\n")
        
        elif source == "files":
            files = [self.listbox_ip_files.get(i) for i in range(self.listbox_ip_files.size())]
            if not files:
                messagebox.showwarning("Предупреждение", "Не выбраны файлы")
                return
            for file_path in files:
                if not os.path.exists(file_path):
                    self.text_ip_output.insert(tk.END, f"Файл не найден: {file_path}\n")
                    continue
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    input_text = f.read()
                ips, saved = self.processor.process_input_to_ips(input_text, output_path)
                all_ips.extend(ips)
                if saved:
                    self.text_ip_output.insert(tk.END, f"IP из {file_path} сохранены в: {output_path}\n")
        
        elif source == "urls":
            urls = [self.listbox_ip_urls.get(i) for i in range(self.listbox_ip_urls.size())]
            if not urls:
                messagebox.showwarning("Предупреждение", "Не указаны URL")
                return
            for url in urls:
                ...
                all_ips.extend(ips)
                if saved:
                    self.text_ip_output.insert(tk.END, f"IP из {url} сохранены в: {output_path}\n")
        
        all_ips = sorted(list(set(all_ips)), key=lambda x: ipaddress.ip_address(x) if '-' not in x else x)
        
        if all_ips:
            self.text_ip_output.insert(tk.END, f"Найдено IP-адресов: {len(all_ips)}\n")
                ...
                if cidrs:
                    ...
                    for cidr in cidrs:
                        try:
                            ...
                            self.text_ip_output.insert(tk.END, f"Ошибка подсчета для {cidr}: {str(e)}\n")
    ...
    def optimize_cidr(self):
        ...
        if source == "text":
            ...
            if not input_text:
                ...
                return
            all_cidrs.extend(self.processor.extract_ips(input_text))
        
        elif source == "files":
            files = [self.listbox_opt_files.get(i) for i in range(self.listbox_opt_files.size())]
            if not files:
                messagebox.showwarning("Предупреждение", "Не выбраны файлы")
                return
            for file_path in files:
                if not os.path.exists(file_path):
                    self.text_opt_result.insert(tk.END, f"Файл не найден: {file_path}\n")
                    continue
                with open(file_path, 'r', encoding='utf-8', errors='ignore') as f:
                    input_text = f.read()
                all_cidrs.extend(self.processor.extract_ips(input_text))
            ...
            urls = [self.listbox_opt_urls.get(i) for i in range(self.listbox_opt_urls.size())]
            if not urls:
                messagebox.showwarning("Предупреждение", "Не указаны URL")
                return
            for url in urls:
        ...
        ipv4_cidrs = [c for c in all_cidrs if ipaddress.ip_network(c, strict=False).version == 4]
        ipv6_cidrs = [c for c in all_cidrs if ipaddress.ip_network(c, strict=False).version == 6]
        ...
        # Вывод сводки
        if show_summary:
            self.text_opt_result.insert(tk.END, "До оптимизации:\n")
            ...
            self.text_opt_result.insert(tk.END, "После оптимизации:\n")
        ...
        self.text_opt_result.insert(tk.END, f"Оптимизированные CIDR (первые 100 из {len(optimized_cidrs)}):\n")
    ...
    def add_ip_files(self):
        files = filedialog.askopenfilenames(title="Выберите файлы")
        if files:
            for file in files:
                if file not in [self.listbox_ip_files.get(i) for i in range(self.listbox_ip_files.size())]:
                    self.listbox_ip_files.insert(tk.END, file)
    
    def clear_ip_files(self):
        self.listbox_ip_files.delete(0, tk.END)
    
    def add_ip_url(self):
        url = self.entry_ip_url.get().strip()
        if url:
            if url not in [self.listbox_ip_urls.get(i) for i in range(self.listbox_ip_urls.size())]:
                self.listbox_ip_urls.insert(tk.END, url)
            self.entry_ip_url.delete(0, tk.END)
    
    def clear_ip_urls(self):
        self.listbox_ip_urls.delete(0, tk.END)

    def add_opt_files(self):
        files = filedialog.askopenfilenames(title="Выберите файлы")
        if files:
            for file in files:
                if file not in [self.listbox_opt_files.get(i) for i in range(self.listbox_opt_files.size())]:
                    self.listbox_opt_files.insert(tk.END, file)
    
    def clear_opt_files(self):
        self.listbox_opt_files.delete(0, tk.END)
    
    def add_opt_url(self):
        url = self.entry_opt_url.get().strip()
        if url:
            if url not in [self.listbox_opt_urls.get(i) for i in range(self.listbox_opt_urls.size())]:
                self.listbox_opt_urls.insert(tk.END, url)
            self.entry_opt_url.delete(0, tk.END)
    
    def clear_opt_urls(self):
        self.listbox_opt_urls.delete(0, tk.END)
